...
newArr.forEach((item) => {
  // looping untuk menggunakan newArr isi array +5
  newArr3.push(item + 5);
});
console.log(newArr3);
...
newArr.map((item, idx) => {
  // looping untuk keluarkan array menjadi console.log sebanyak index sama kek forEach tapi bisa return array baru
  console.log(item, idx);
});

for (let item of newArr) {
  // looping untuk mengetahui semua isi array
  console.log(item);
}

// for(let item in arr){ // looping untuk mengetahui  key di object
//     console.log(item);
// }

// for(let i = 0 ; i< arr.length ; i++){ // sama seperti "let item in arr"
//console.log(arr[i])
// }
